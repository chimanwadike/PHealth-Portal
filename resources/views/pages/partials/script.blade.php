...
<script type="module">
	window.Echo.private('notification.'+{{auth()->user()->id}}).notification((notification) => {
	    $.ajaxSetup({
	        cache: false
	    });

	    $.ajax({
	        url: "{{route('reload_notification', auth()->user())}}",
	        success: function (data) {
	        	...
	        	$('#number').html(++number);
	        	$('.notification-box').removeClass('hide-notification')
	        },
	    });
	});
</script>
...
<script type="text/javascript">
	...
	$('#notification-box').on('click', '.delete-notification', function(event){
		event.preventDefault();

        var button = $(event.target); // Button that triggered the modal
        var notification_id = button.data('value'); // Extract info from data-* attributes
	    ...
	    $.ajax({
	        url: '{{ env('APP_URL') }}notification/delete/'+notification_id,
	        success: function (data) {
	            $('#notification-box').html(data);

	            $('.notification-box').addClass('hide-notification')
	        },
	    });
    })
 	...
 	function isclicked(){
 		...
 		if(number > 0){
		    ...
		    $.ajax({
		        url: '{{ route("clear_notification", auth()->user()) }}',
		        success: function (data) {
		        	$('#number').html(0);
		        	$('#number').val(0);
		        	$('.notification-box').addClass('hide-notification')
