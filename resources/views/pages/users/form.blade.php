<form method="POST" novalidate="novalidate" action="{{ (isset($user)) ? route('users.update', $user->id):route('users.store') }}">
    ...
    @elseif(auth()->user()->hasRole('coordinator'))
        <div class="row">
            <div class="col-lg-6 col-md-6 col-xs-12 col-sm-12 mt-3">
                <label>Full Name:</label>
                <input value='{{ (isset($user) ? $user->name : "") }}' type="text" class="form-control" disabled autofocus>
            </div>

            <div class="col-lg-6 col-md-6 col-xs-12 mt-3">
                <label>Sex:</label>
                <select class="form-control m-select2" id="kt_select2_sex" disabled>
                    <option></option>
                    <option value="male" @if(old("sex", (isset($user)) ? $user->sex : "") == "male") {{ "selected" }} @endif>Male</option>
                    <option value="female" @if(old("sex", (isset($user)) ? $user->sex : "") == "female") {{ "selected" }} @endif>Female</option>
                </select>
        ...
        <div class="row">
            <div class="col-lg-6 col-md-6 col-xs-12 col-sm-12 mt-3">
                <label>Email Address:</label>
                <input value="{{ old('email', (isset($user)) ? $user->email : "") }}" type="text" class="form-control" disabled autofocus>
                @if($errors->has('email'))
                    <strong class="text-danger">{{ $errors->first('email') }}</strong>
                @endif
            </div>
            ...
            <div class="col-lg-6 col-md-6 col-xs-12 col-sm-12 mt-3">
                <label>Phone Number</label>
                <input value="{{ old('phone', (isset($user)) ? $user->phone : "") }}" type="text" class="form-control" disabled autofocus>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-lg-6 col-md-6 col-xs-12 col-sm-12 mt-3">
                ...
                <textarea disabled class="form-control" name="address">{{ old('address', (isset($user)) ? $user->address : "") }}</textarea>
            </div>

            <div class="col-lg-6 col-md-6 col-xs-12 col-sm-12 mt-3">
                <label>Role</label>
                <select disabled class="form-control m-select2" id="role" name="role">
                    <option></option>
                    @foreach($roles as $role)
                        <option @if(old('role', (isset($user)) ? $user->roles()->first()->id : "") == $role->id) {{ 'selected' }} @endif value="{{ $role->id }}">{{ $role->display_name }}</option>
                    @endforeach
